...
(function (jQuery) {
   ...
   const pages = [
      ...
      { title: "About", url: "about.html", keywords: ["about", "info", "us"] },
  ...
  $(document).ready(function() {
      $('#searchInput').on('keyup', function() {
          ...
          const results = pages.filter(page => {
              return page.title.toLowerCase().includes(query) || page.keywords.some(keyword => keyword.includes(query));
          });
          ...
          if (results.length === 0) {
              ...
              results.forEach(result => {
                  const resultItem = `<div><a href="${result.url}">${result.title}</a></div>`;
